...
export default function Home(){
  ...
  useEffect(() => {
    ...
    <section>
        ...
        <h1 className="text-center md:text-3xl text-2xl font-bold font-mono mb-4 p-2 border-b-[2px] border-b-black">Notice Board</h1>
        <div className="grid lg:grid-cols-3 md:grid-cols-2 gap-4">
        {/* Students Section */}
            <section className="bg-red-300 opacity-90 p-4 rounded-lg shadow-md">
            <h1 className="text-center md:text-2xl p-2 border-b-[2px] border-b-black text-xl font-semibold font-mono mb-4">Students</h1>
            {studentNotices.map((notice, index) => (
                <div key={index} className="mb-4 pb-6 border-b-[1px] relative border-b-white p-2"> {/* Added padding-bottom */}
                <p
                ...
                <br></br>
                <div className="text-md text-gray-700  font-semibold absolute bottom-1 right-1 flex items-center space-x-1"> {/* Adjusted bottom position */}
                    <FaCalendarAlt className="mr-1" /> <span>{notice.date}</span>
                </div>
                </div>
            ))}
            </section>
            ...
            {/* Teachers Section */}
            <section className="bg-red-300 opacity-90 p-4 rounded-lg shadow-md">
            <h1 className="text-center md:text-2xl p-2 border-b-[2px] border-b-black text-xl font-semibold font-mono mb-4">Teachers</h1>
            ...
            {/* Important Section */}
            <section className="bg-red-300 opacity-90 p-4 rounded-lg shadow-md">
            <h1 className="text-center md:text-2xl p-2 border-b-[2px] border-b-black text-xl font-semibold font-mono mb-4">Important</h1>
      ...
      <h2 className="text-center md:text-3xl text-2xl font-bold font-mono mb-4 p-2 border-b-[2px] border-b-black">Gallery Of Arts</h2>
      <div className="grid grid-cols-2 lg:grid-cols-4 md:grid-cols-4  gap-4">
        {images.slice(0, visibleCount).map((image, index) => (
          <div key={index} className="gallery-item p-2">
            ...
            />
            <h3 className="mt-2 font-semibold text-md font-sans">{image.title}</h3>
          </div>
        ))}
      </div>
      ...
      <div className="values rounded-md bg-red-100 m-2 p-4 shadow-lg border border-gray-300">
        <h1 className="text-center md:text-3xl text-2xl font-bold font-mono mb-4">Our Values</h1>
        <div className="grid lg:grid-cols-3 md:grid-cols-2 gap-4">
          {values.map((value, index) => (
            ...
            >
              <h2 className="text-center md:text-2xl text-xl font-semibold font-mono mb-4">{value.title}</h2>
        ...
        {/* stats */}
        <div className="values rounded-md bg-red-100 m-2 p-4">
      <h1 className="text-center md:text-3xl text-2xl font-bold font-mono mb-4">Our Journey</h1>
      <div className="grid content-center lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 gap-4">
        {displayCounts.map((data, index) => (
          <div key={index} className={`${data.color} p-4 rounded-lg shadow-md flex flex-col items-center`}>
            <h2 className="text-xl font-semibold">{data.displayedCount}+</h2>
